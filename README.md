...
```bash
DockerExample$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
DockerExample$ sam local start-api
DockerExample$ curl http://localhost:3000/classify_digit
```
...
```bash
DockerExample$ sam logs -n InferenceFunction --stack-name DockerExample --tail
```
...
```bash
aws cloudformation delete-stack --stack-name DockerExample
```
